...
async def onlineUsers_handler(request: Request):
    return JSONResponse({
        'status': 200,
...
async def verifiedStatus_handler(request: Request):
    ...
    if user_data and (user_data['privileges'] & KurikkuPrivileges.Normal) == KurikkuPrivileges.Normal:
        verifiedStatus = 1
